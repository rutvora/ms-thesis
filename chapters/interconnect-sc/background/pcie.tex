...
Similar to network protocols, PCIe utilises a layered architecture consisting of the transaction layer, data link layer, and physical layer, which are implemented in a PCIe controller (see \Cref{fig:pcie-controller}).
...
PCIe enforces strict ordering rules for each of these types, which we outline in \Cref{tab:pcie-transaction-ordering-rules}.
